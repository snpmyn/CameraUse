...
class Luban @JvmOverloads constructor(
    ...
    private suspend fun compressInternal(context: Context, input: Uri, output: File): File =
    ...
    private suspend fun compressInternal(input: File, output: File): File =
        withContext(Dispatchers.IO) {
            val options = BitmapFactory.Options().apply { inJustDecodeBounds = true }
...
suspend fun List<Uri>.compressTo(
    context: Context,
    outputDir: File = context.cacheDir
): List<Result<File>> {
    return Luban.compress(context, this, outputDir)
